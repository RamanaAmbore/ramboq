...
- **Per-symbol price-driven simulator with spread-aware chase engine.** Positions-only by design. Ships 6 book-wide stress scenarios (`generic-crash` / `-euphoria`, `extreme-crash` / `-euphoria`, `wild-swings`, `random-walk`) + 7 market-state presets (`pre_open`, `at_open`, `mid_session`, `pre_close`, `at_close`, `post_close`, `expiry_day`) so time-aware agents fire realistically. Every position derives `bid` / `ask` from a tunable spread; the chase engine re-quotes open orders each tick and marks them `FILLED` when the market crosses or `UNFILLED` after `simulator.chase_max_attempts`. Filled positions are removed from the book so the operator watches it shrink live. [`ADMIN_GUIDE.md`](ADMIN_GUIDE.md) walks the whole flow.
...
- **Paper-trade order log.** Sim actions record an `AlgoOrder` row with `initial_price = side-aware limit (SELL→bid, BUY→ask)`, then chase progress bumps `attempts` + rewrites `detail` live until the order flips to `FILLED` / `UNFILLED`. The LogPanel's Order tab colours each row by terminal state (FILLED green / UNFILLED red / OPEN amber) with a `chase: #N` chip — operators see the chase in action.
...
- **Three execution modes, branch-gated.** Mode 1 — fabricated quotes + paper trade engine — runs only on dev (the Simulator). Mode 2 — real Kite quotes + paper trade engine fed by `LiveQuoteSource`, with `basket_margin` validation per order — is the prod default. Mode 3 — real broker placement — is opt-in per-action via `execution.live.<action>` flags in [/admin/settings](https://dev.ramboq.com/admin/settings) (start with `cancel_order`, finish with `place_order`). The chase / fill / modify / unfilled lifecycle is a single `PaperTradeEngine` shared across mode 1 and mode 2 — only the quote source differs. Every alert is auto-tagged `[PAPER]` / `[MIXED]` / nothing in Telegram + email subjects so the operator knows at a glance whether a fired agent caused a real broker order.
