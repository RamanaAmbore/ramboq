...
def profile_layout(mode="view", profile_data=None):
    ...
    if mode == "view":
        ...
        st.text(f"Contact Number: {profile_data['contact_number']}, Backup: {profile_data.get('backup_contact_number','')}")
        return
    ...
    with st.form(key=f"profile_form_{mode}"):
        ...
        nominee_address = st.text_area("Nominee Address", value=profile_data.get("nominee_address","") if profile_data else "")
        nominee_contact = st.text_input("Nominee Contact", value=profile_data.get("nominee_contact","") if profile_data else "")

        st.markdown("**Address Details**")
        address_line1 = st.text_input("Address Line 1", value=profile_data.get("address_line1","") if profile_data else "")
        address_line2 = st.text_input("Address Line 2 (Optional)", value=profile_data.get("address_line2","") if profile_data else "")
        ...
        contact_number = st.text_input("Contact Number", value=profile_data.get("contact_number","") if profile_data else "")
        backup_contact_number = st.text_input("Backup Contact Number (Optional)", value=profile_data.get("backup_contact_number","") if profile_data else "")
